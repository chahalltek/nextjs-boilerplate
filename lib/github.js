...
/**
 * Read a file from the repo via Contents API.
 * Returns { contentBase64, sha } or null if not found.
 */
export async function getFile(path) {
  ...
  if (!r.ok) {
    throw new Error(`GitHub get failed ${r.status}: ${j.message || txt}`);
  }

  // GitHub returns base64 in "content" with newlines; strip them.
 ...
 * params: { path, contentBase64, message, sha? }
 */
export async function commitFile({ path, contentBase64, message, sha }) {
  ...
  const body = {
    ...
    content: contentBase64,     // JSON body, not query string
    branch: GH_BRANCH,
...
export async function deleteFile({ path, message }) {
  ...
  const body = {
    message: message || `chore(admin): delete ${path}`,
    sha,
    branch: GH_BRANCH,
  };
  ...
  if (!r.ok) {
    throw new Error(`GitHub delete failed ${r.status}: ${j.message || txt}`);
  }
  return { ok: true, commit: j.commit?.sha, path, branch: GH_BRANCH };
}
