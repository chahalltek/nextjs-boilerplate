...
/** Read a file (returns { contentBase64, sha } or null). */
...
/** List a directory (returns array of {name,path,type,sha} or null if 404). */
export async function listDir(path) {
  ...
  if (!r.ok) throw new Error(`GitHub list failed ${r.status}: ${j.message || txt}`);
  if (!Array.isArray(j)) return []; // GitHub returned a single file object

  return j.map(({ name, path: p, type, sha }) => ({ name, path: p, type, sha }));
}
