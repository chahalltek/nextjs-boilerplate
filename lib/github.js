...
/** Read file text + sha. Returns null if missing. */
...
/** Create or update a file with a commit (uses sha when provided). */
...
export async function deleteFile(path, message = `chore: delete ${path}`) {
  const existing = await getFile(path).catch(() => null);
  if (!existing) return { ok: true, skipped: "missing" };

  const { owner, repo } = repoParts();
  const body = { message, sha: existing.sha, branch: branch() };
  const name = process.env.GITHUB_COMMITTER_NAME;
  const email = process.env.GITHUB_COMMITTER_EMAIL;
  if (name && email) body.committer = { name, email };
...
/* --- Compatibility alias used by your routes --- */
