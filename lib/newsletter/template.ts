...
const LOGO_URL =
  process.env.NEWSLETTER_LOGO_URL || "https://heyskolsister.com/logo-email.png";
...
export function renderNewsletterEmail(draft: NewsletterDraft): string {
  const subject =
    draft.subject || "Your weekly Hey Skol Sister rundown!";
...
<head>
  ...
  <style>
    ...
    .nav a { color:#ffffff; opacity:.85; font-size:13px; margin:0 8px; }
...
function markdownToHtml(md: string): string {
  ...
  md = md
    .split(/\n{2,}/)
    .map((chunk) => {
      // Already converted blocks? keep as is
