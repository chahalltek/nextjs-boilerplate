...
export const runtime = "nodejs"; // Resend + Buffer need Node
...
function htmlForRoster(teamName: string, rules: Body["rules"], players: Body["players"]) {
  ...
  <div style="font-family:system-ui,Segoe UI,Arial,sans-serif;line-height:1.6">
    ...
    <p style="margin-top:16px;color:#555">CSV is attached for importing to your notes/sheets.</p>
  </div>`;
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!email || !Array.isArray(players) || players.length === 0) {
      return NextResponse.json({ ok: false, error: "email and players are required" }, { status: 400 });
    }
    ...
    const { error } = await resend.emails.send({
      from: "Hey Skol Sister <hello@heyskolsister.com>", // must be a verified Resend domain/sender
      to: email,
      ...
      attachments: [
        {
          filename: `${teamName.replace(/\s+/g, "-").toLowerCase()}-roster.csv`,
    ...
    // (Optional) If you decide to persist/schedule weekly later, do it here.
