// app/admin/lineup-lab/overrides/page.tsx
...
export default async function OverridesAdmin() {
  const week = 1;
  const current = await getOverrides(week);

  return (
    <main className="container max-w-4xl py-10 space-y-6">
      <h1 className="text-3xl font-bold">Lineup Lab — Admin Overrides</h1>
      ...
      <form action={actionSave} className="grid gap-3 rounded-xl border border-white/10 bg-white/5 p-4">
        ...
        <textarea
          name="pointDelta"
          className="min-h-24 bg-transparent border border-white/10 rounded p-2"
